...
def extrage_date_structurate(text_brut):
    ...
    while i < len(linii) - 2:
        ...
        if len(nume_curent) < 4 or any(x in nume_upper for x in ["MEDICALE", "PAGINA", "BULETIN", "DATA", "NUME", "ANALIZA", "REZULTAT"]):
            i += 1
            continue
        ...
        if re.search(r'^[-a-zA-Z\(\*\d]', nume_curent):
            ...
            match_valoare = re.search(r'([<>]?\d+[\.,]?\d*)(?:\s*10\^|\s*[a-zA-Z/%])?', urmatoarea_linie)
            
            if match_valoare and ('-' in a_treia_linie or '/' in a_treia_linie or 'ref' in a_treia_linie.lower() or '<' in a_treia_linie or '>' in a_treia_linie):
...
def extrage_date_pacient(text_brut):
    ...
    match_data = re.search(r'(?:recoltare|cerere)[\s:]*(\d{2}[./-]\d{2}[./-]\d{4})', text_brut, re.IGNORECASE)
    if match_data:
        date_pacient["data_recoltare"] = match_data.group(1).replace('.', '-').replace('/', '-')

    # 4. Extragere Nume
    match_nume = re.search(r'Nume[\s:]*([A-Za-z\-]+)\s+([A-Za-z\-]+)', text_brut, re.IGNORECASE)
    if match_nume:
