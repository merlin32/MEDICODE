...
valoare_reala = st.floats(min_value=-10_000, max_value=10_000,
...
nume_biomarker = st.text(min_size=1, max_size=50,
...
class TestValoriNormale(unittest.TestCase):
    ...
    def test_valoare_normala_nu_apare_in_output(self, mock_db_class,
                                                u_id, s_id, nume,
                                                v_min, v_max, val):
        """
        ...
        fake_row = {
            'id_utilizator': u_id, 'id_sesiune': s_id,
            'nume_biomarker': nume,
...
class TestValoriAnormale(unittest.TestCase):
    ...
    @settings(max_examples=200)
    @patch('anomalies_detector.DatabaseConnection')
    def test_valoare_anormala_apare_in_output(self, mock_db_class,
                                              u_id, s_id, nume,
                                              v_min, v_max, val):
        """
...
class TestDeviere(unittest.TestCase):
    ...
    @settings(max_examples=200)
    @patch('anomalies_detector.DatabaseConnection')
    def test_devierea_este_intotdeauna_pozitiva(self, mock_db_class,
                                                u_id, s_id,
                                                v_min, v_max, val):
        """
    ...
    @given(
        v_min=st.floats(min_value=0.01, max_value=10_000,
                        allow_nan=False, allow_infinity=False),
        v_max=st.floats(min_value=0.01, max_value=10_000,
                        allow_nan=False, allow_infinity=False),
        val=valoare_reala
    )
    @settings(max_examples=200)
    @patch('anomalies_detector.DatabaseConnection')
    def test_devierea_cu_referinte_pozitive(self, mock_db_class,
                                            v_min, v_max, val):
        """
        ...
        if result:
            ...
            deviere = float(biomarker['deviere'].replace('%', ''))
            self.assertGreaterEqual(deviere, 0.0)
            self.assertFalse(deviere != deviere,  # NaN check
                             "Devierea nu trebuie sa fie NaN!")
...
class TestGrupare(unittest.TestCase):
    ...
    @patch('anomalies_detector.DatabaseConnection')
    def test_numar_utilizatori_in_output(self, mock_db_class,
                                         u_ids, v_min, v_max, val):
        """
        ...
        rows = [
            {
                'id_utilizator': u_id, 'id_sesiune': i + 1,
        ...
        self.assertEqual(id_uri_in_output, set(u_ids),
    ...
    def test_numar_sesiuni_per_utilizator(self, mock_db_class, s_ids):
        ...
        rows = [
            {
                'id_utilizator': 1, 'id_sesiune': s_id,
                'nume_biomarker': 'Glicemie',
        ...
        self.assertEqual(len(sesiuni_in_output), len(s_ids),
...
class TestJsonSerializabil(unittest.TestCase):
    ...
    @settings(max_examples=200)
    @patch('anomalies_detector.DatabaseConnection')
    def test_output_este_json_serializabil(self, mock_db_class,
                                           u_id, s_id, v_min, v_max, val):
        """
